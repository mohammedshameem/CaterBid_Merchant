...
public class CurrentlyLoggedUserDAO implements CurrentlyLoggedUser {
		...
		synchronized (DBHelper.lock) {
			...
			try {
				...
				if (UtilValidate.isNotNull(emailsignupobject.getData()
					...
					values.put(CurrentlyLoggedUser.LOGINTYPE, emailsignupobject
							.getData().getUser().getLoginType());
				if (UtilValidate.isNotNull(emailsignupobject.getData()
						.getUser().getUdId()))
					values.put(CurrentlyLoggedUser.UDID, emailsignupobject
							.getData().getUser().getUdId());
			...
			try {
				if (cursor.getCount() > 0) {
					while (cursor.moveToNext()) {
						...
						//loggeduserObject.setUser(userholder);
						if(cursor.getString(cursor.getColumnIndex(CurrentlyLoggedUser.UDID)) != null){
							userholder.setUdId(cursor.getString(cursor.getColumnIndex(CurrentlyLoggedUser.UDID)));
						}
						if(cursor.getString(cursor.getColumnIndex(CurrentlyLoggedUser.LOGINTYPE)) != null){
							userholder.setLoginType(cursor.getString(cursor.getColumnIndex(CurrentlyLoggedUser.LOGINTYPE)));
						}
						loggeduserObject.setUser(userholder);
					}
				}
	...
	public String getUdId() {
			...
			try {
				if (cursor.getCount() > 0) {
					while (cursor.moveToNext()) {
						udId = cursor.getString(cursor.getColumnIndex(User.UDID));
					}
				}
	...
	public String getLoginType() {
		...
		try {
			if (cursor.getCount() > 0) {
				while (cursor.moveToNext()) {
					logInType = cursor.getString(cursor.getColumnIndex(CurrentlyLoggedUser.LOGINTYPE));
	...
	public void InsertCurrentlyloggedUserDetails(UserTokenHolder userTokenHolder) {
		...
		synchronized (DBHelper.lock) {
			...
			try {
				...
				if (UtilValidate.isNotNull(userTokenHolder.getUser().getLoginType()))
					values.put(CurrentlyLoggedUser.LOGINTYPE, userTokenHolder.getUser().getLoginType());
				if (UtilValidate.isNotNull(userTokenHolder.getUser().getUdId()))
					values.put(CurrentlyLoggedUser.UDID, userTokenHolder.getUser().getUdId());
				db.insert(DB_TABLE_NAME, null, values);
				db.setTransactionSuccessful();
			} catch (Exception e) {
				Log.e(TAG, "Exception while inserting in to user table", e);
